...
function mmapAlloc(size) {
  ...
  var ptr = abort(
      'malloc was not included, but is needed in mmapAlloc. Adding "_malloc" to EXPORTED_FUNCTIONS should fix that. This may be a bug in the compiler, please file an issue.');
...
var FS = {
    ...
    for (var i = 0; i < parts.length; i++) {
      ...
      if (!islast || opts.follow) {
        ...
        while (FS.isLink(current.mode)) {
          ...
          var lookup =
              FS.lookupPath(current_path, {recurse_count: opts.recurse_count});
          current = lookup.node;
          if (count++ > 40) {
            throw new FS.ErrnoError(32)
          }
  ...
  rename: function(old_path, new_path) {
    ...
    try {
      ...
      err('FS.trackingDelegate[\'willMovePath\'](\'' + old_path + '\', \'' +
    ...
    try {
      ...
      err('FS.trackingDelegate[\'onMovePath\'](\'' + old_path + '\', \'' +
          new_path + '\') threw an exception: ' + e.message)
    }
  },
  rmdir: function(path) {
    ...
    try {
      ...
      err('FS.trackingDelegate[\'willDeletePath\'](\'' + path +
    ...
    try {
      ...
      err('FS.trackingDelegate[\'onDeletePath\'](\'' + path +
          '\') threw an exception: ' + e.message)
    }
  },
  ...
  readlink: function(path) {
    ...
    return PATH_FS.resolve(
        FS.getPath(link.parent), link.node_ops.readlink(link))
  },
  ...
  open: function(path, flags, mode, fd_start, fd_end) {
    ...
    try {
      ...
      err('FS.trackingDelegate[\'onOpenFile\'](\'' + path +
  ...
  write: function(stream, buffer, offset, length, position, canOwn) {
    ...
    try {
      ...
      err('FS.trackingDelegate[\'onWriteToFile\'](\'' + stream.path +
          '\') threw an exception: ' + e.message)
    }
    return bytesWritten
  },
  ...
  mmap: function(stream, address, length, position, prot, flags) {
    ...
    return stream.stream_ops.mmap(
        stream, address, length, position, prot, flags)
  },
  msync: function(stream, buffer, offset, length, mmapFlags) {
    ...
    FS.ErrnoError = function ErrnoError(errno, node) {
      ...
      if (this.stack) {
        Object.defineProperty(
            this, 'stack', {value: (new Error).stack, writable: true});
    ...
    openRequest.onsuccess = function openRequest_onsuccess() {
      ...
      paths.forEach(function(path) {
        ...
        getRequest.onsuccess = function getRequest_onsuccess() {
          ...
          FS.createDataFile(
              PATH.dirname(path), PATH.basename(path), getRequest.result, true,
              true, true);
          ok++;
          if (ok + fail == total) finish()
        };
...
function _strftime(s, maxsize, format, tm) {
  ...
  var EXPANSION_RULES_2 = {
    ...
    '%j': function(date) {
      return leadingNulls(
          date.tm_mday +
              __arraySum(
                  __isLeapYear(date.tm_year + 1900) ? __MONTH_DAYS_LEAP :
    ...
    '%U': function(date) {
      ...
      if (compareByDay(firstSunday, endDate) < 0) {
        var februaryFirstUntilEndMonth =
            __arraySum(
                __isLeapYear(endDate.getFullYear()) ? __MONTH_DAYS_LEAP :
                                                      __MONTH_DAYS_REGULAR,
                endDate.getMonth() - 1) -
            31;
  ...
  for (var rule in EXPANSION_RULES_2) {
    if (pattern.indexOf(rule) >= 0) {
      pattern =
          pattern.replace(new RegExp(rule, 'g'), EXPANSION_RULES_2[rule](date))
    }
  }
